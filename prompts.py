...
CHAT_AGENT_INSTRUCTIONS = (
    ...
    "💰 ВАЖНО О ДЕНЬГАХ:\n"
"— Победитель получает 1000 единиц = 10 МИЛЛИОНОВ РУБЛЕЙ для распределения.\n"
"— Это огромные деньги! На них можно: купить квартиру, открыть бизнес, обеспечить семью на годы.\n"
"— ТЫ ДОЛЖЕН распределить ВСЕ 1000 единиц в своем `allocation` (можешь включить себя).\n"
...
def build_president_prompt_prompt(person: Person, context: str,  conflict_notice: str, topic: str, own_lines: str, reactions_block, new_point_line):
    ...
    💸 ПРАВИЛА РАСПРЕДЕЛЕНИЯ ДЕНЕГ (КРИТИЧЕСКИ ВАЖНО):
    - У тебя есть 1000 единиц (10 млн рублей) для распределения
    - ТЫ ОБЯЗАН распределить ВСЕ 1000 единиц между участниками (включая себя). Укажи это в соответствующем поле ответа.
    - Это твоя ПРЕДВЫБОРНАЯ ПРОГРАММА - все видят, насколько ты щедр

    Чётко раздели две мысли:
    1) ЧТО ты хочешь, чтобы решил президент;
...
💰 КРИТИЧЕСКИ ВАЖНО - ЭТО ТВОЙ ШАНС НА ДЕНЬГИ:
    ...
    - Да, у тебя есть ценности, НО 10 миллионов - это огромные деньги
...
def build_distribution_prompt_prompt(person: Person, money: int, context: str) -> str:
    return f"""Ты стал президентом обсуждения и получил {money} денег. Проанализируй предложения участников по распределению средств:\n{context}\n\nРеши, кому и сколько выдать. Верни JSON вида, примерный вид:\n{{\n  \"distribution\": {{\"Спикер 1\": 500, \"Нужды на образование\": 100}},\n  \"comment\": \"пару слов почему так\"\n}}"""